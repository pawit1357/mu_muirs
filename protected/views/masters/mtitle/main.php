<form id="Form1" method="POST" enctype="multipart/form-data"
	...
	<div class="portlet light">
				<div class="portlet-title">
					...
					<div class="actions">
					<?php echo (UserLoginUtils::canCreate($_SERVER['REQUEST_URI']) == false)? "":  CHtml::link('เพิ่มข้อมูล',array('MTitle/Create'),array('class'=>'btn btn-default btn-sm'));?>
					</div>
				</div>
...
<a title="Edit" class="fa fa-edit"
									href="<?php echo Yii::app()->CreateUrl('MTitle/Update/id/'.$data->id)?>"></a>
<?php }?>
<?php if(UserLoginUtils::canDelete( $_SERVER['REQUEST_URI'])){?>
<a title="Delete" onclick="return confirm('ต้องการลบข้อมูลใช่หรือไม่?')"
									class="fa fa-trash"
									href="<?php echo Yii::app()->CreateUrl('MTitle/Delete/id/'.$data->id)?>"></a>
<?php }?>	
