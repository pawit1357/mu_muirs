...
class SiteController extends CController
{
    ...
    public function actionReg()
    {
        if (isset($_POST['UsersLogin'])) {
            ...
            $model->role_id = 2; // staff
            $model->status = "I"; // staff
            ...
            // Notify//
            $model = new Notify();
            $model->report_type = 0; // 1:Acicent&Investigate,2:Incident,3:Accident
            $model->title = 'ลงทะเบียนใช้งานระบบ';
            $model->remark = 'ลงทะเบียนใช้งานระบบ';
            $model->isRead = false;
            $model->create_date = date("Y-m-d H:i:s");
            $model->save();
            // End-Notify//
